...
{
    template <typename R, typename ...A>
    struct hash<wee::delegate<R (A...)> >
    {
        size_t operator()(wee::delegate<R (A...)> const& d) const noexcept
        {
            auto const seed(hash<void*>()(d.object_ptr_));
            
            return hash<typename wee::delegate<R (A...)>::stub_ptr_type>()(d.stub_ptr_) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
        }
    };
}
