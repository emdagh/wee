...
int main(int, char**) {
    ...
    std::vector<std::string> strings = string_split_on_all(tmp, ' ');
    std::vector<size_t> sizes;
    /*std::transform(strings.begin(), strings.end(), std::back_inserter(sizes), [] (const std::string& i) {
        return lexical_cast<size_t>(i);
    });*/
    
    for(size_t i=0; i < strings.size(); i++) {
        int64_t j = lexical_cast<int64_t>(strings[i]);
        if(j < 1 || 1024 <= j) { 
            std::cout << "INPUT_ERROR" << std::endl;
            return 0;
        }
        sizes.push_back(lexical_cast<size_t>(strings[i]));
    }

    std::getline(std::cin, tmp);
    if(lexical_cast<int64_t>(tmp) < 1) {
        std::cout << "INPUT_ERROR" << std::endl;
        return 0;
    }
    size_t n = lexical_cast<int>(tmp);
    VALIDATE_CIN();
    ...
    while(std::getline(std::cin, tmp)) {
        ...
        sequence.emplace_back(lexical_cast<size_t>(tmp));
    }
    ...
    try {
        //std::cout << "bucket=" << bucket_of(c.address_of(2), sizes) + 1 << std::endl;
        auto addr = std::distance(c.begin(), c.find(sequence.back()));
        //std::cout << bucket_of(c.address_of(sequence.back()), sizes) + 1 << std::endl;
...
/** here is the original (naive) implementation */
...
typedef std::vector<uint32_t> sequence_type;

enum class state : int8_t { // max 2^6 bits
    kNew = -4,
    kOutside,
    kError,
    kMaxState
...
int do_sequence(const std::vector<int>& sizes, const sequence_type& sequence) 
{
    /**
     * determine the ending index (size - 1) of the array in which we store our data. (outside)
     */
    size_t outside = std::accumulate(sizes.begin(), sizes.end(), 0) - 1;
    /**
     * next, we use a deque for readability purposes (it has push_front)
     */
    std::deque<sequence_type::value_type> items; 
    /**
     * The sentry iterator will keep track of the last item taken off of the shelf.
     */
    auto sentry = items.end();
    for(auto i: sequence) {
        if(auto it = std::find(items.begin(), items.end(), i); it != items.end()) {
            std::iter_swap(items.begin(), it); 
            sentry = it;
        } else {
            items.push_front(i);
            sentry = items.begin();
    ...
    size_t diff = std::distance(items.begin(), sentry);
    if(diff == 0) 
        return static_cast<int>(state::kNew);
    if(diff > outside) 
        return static_cast<int>(state::kOutside);

    for(size_t i=0, s=0; i < sizes.size(); i++) {
        s = s + sizes[i];
        if(diff < s) return i + 1; // arrays don't start at 1... 
    }
    return static_cast<int>(state::kError);
}

int test(const std::vector<int>& sizes, const sequence_type& seq) {
    int err = do_sequence(sizes, seq);


    switch(static_cast<state>(err)) {
        case state::kNew:       std::cout << "NEW" << std::endl; break;
        case state::kOutside:   std::cout << "OUTSIDE" << std::endl; break;
        case state::kError:     std::cout << "ERROR" << std::endl; break;
        case state::kMaxState:  std::cout << "ERROR" << std::endl; break;
...
int main(int, char**) {
    ...
    std::string str_sizes;
    std::cin.clear();
    std::getline(std::cin, str_sizes);
    if(!std::cin.good()) { std::cout << "INPUT_ERROR 1" << std::endl; return 0; }
    
    std::vector<std::string> vec_sizes = string_split_on_all(str_sizes, ' ');
    std::vector<int> sizes;

    std::transform(vec_sizes.begin(), vec_sizes.end(), std::back_inserter(sizes), [] (const std::string& s) {
        return lexical_cast<int>(s);
    });
    
    for(auto s: sizes) {
        if(s < 1) {
            std::cout << "INPUT_ERROR 2" << std::endl;
            return 0;
        }
    }
    ...
    std::cin >> n;
    if(n < 1 || (!std::cin.good())) {
        std::cout << "INPUT_ERROR 3" << std::endl;
        return 0;
    }
    ...
    for(auto s: sizes) {
        if(s < 1) {
            std::cout << "INPUT_ERROR 4" << std::endl;
            return 0;
        }
    }
    ...
    /*assert(test({2,2,4}, { 1,2,3,4,5,6,2 }) == 3);
    assert(test({2,4}, {83674,
                28736,
                398,
                109283,
                64832}) == static_cast<int>(state::kNew));;
    assert(test({8}, { 1,2,3,4,5,6,7,8,9,1}) == static_cast<int>(state::kOutside));*/
    return 0;
}
